...
import { GardenChip } from '../GardenChip/GardenChip';
...
/* ── Status chip fills (keyed by status) ── */
...
/**
 * Slot + GardenChip: the outer div is the fixed-width column slot (so adjacent
 * columns stay in vertical lanes), the inner GardenChip hugs its own text.
 */
const StatusCell: React.FC<{ status: string; slotWidth: number }> = ({ status, slotWidth }) => (
  <div style={{ width: slotWidth, display: 'flex', alignItems: 'center', flexShrink: 0 }}>
    <GardenChip
      size="small"
      label={status}
      iconMode="leading"
      leadingIcon={<StatusIcon status={status} />}
      backgroundColor={STATUS_BG[status] ?? STATUS_BG['In progress']}
    />
  </div>
);
...
const largeCols: TableColumn[] = [
  ...
  { key: 'source', header: 'Source', width: 121 },
  {
    ...
    header: 'Status',
    width: 110,
    render: (val: string) => <StatusCell status={val} slotWidth={110} />,
  },
];
...
const smallCols: TableColumn[] = [
  ...
  { key: 'apy', header: 'APY', width: 64 },
  {
    ...
    width: 80,
    /**
     * Odd rows carry the overlay-mid (50%) fill, so the end-date text uses
     * text-primary for stronger contrast. Even rows step down to text-secondary
     * for a muted, subordinate look. Semantic tokens so both themes flip
     * correctly: dark grey/muted in light mode, white-100/white-50 in dark mode.
     */
    render: (val: string, _row, rowIdx) => (
      <span
        className="garden-h4-regular"
        style={{ color: rowIdx % 2 === 1 ? 'var(--color-text-primary)' : 'var(--color-text-secondary)' }}
      >
        {val}
      </span>
    ),
  },
  {
    ...
    render: () => (
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
        <path d="M4 6.5H7M9 6.5H12M4 9.5H7M9 9.5H12" stroke="var(--color-text-secondary)" strokeWidth="1" strokeLinecap="round" />
      </svg>
    ),
  },
];
